...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscribe','white'),color('YOU','red'),color('TUBE','white'),color('bynx ff','yellow'))
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\Selamat datang di group *${mdata.subject}*  . jangan lupa baca deskripsi group :D semoga betah di sini NGAB :) \n ┗━━━⊱  ⸨ *𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧* ⸩  ⊰━━━━┛`
				...
				teks = `akhirnya beban group berkurang 𝟭, @${num.split('@')[0]} semoga di temani oleh dakjal :D`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
        ...
        case 'trendtwit':
           ...
           case 'demote':
                ...
                case 'tagall3':
                        ...
                        client.sendMessage(from, '╔══✪〘 TAG SEMUA MEMBER 〙✪══\n╠➥'+teks+'╚═〘 *𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧* 〙', text, {detectLinks: false, quoted: mek})
                                        await limitAdd(sender)
                    break
                        case 'tagall4':
                                client.updatePresence(from, Presence.composing)
                ...
                if (args[0] === 'enable') {
				 ...
				 case 'otagall':
                                client.updatePresence(from, Presence.composing)
                                        ...
                                        mentions('╔══✪〘 TAG ALL SEMUA MEMBER 〙✪══\n╠➥'+teks+'╚═〘 *𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧* 〙', members_id, true)
                                        break
                case 'otagall2':
                                client.updatePresence(from, Presence.composing)
                                        if (!isGroup) return reply(ind.groupo())
