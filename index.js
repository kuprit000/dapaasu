/*
]=====> RIU ADMIN <=====[ ]=====> YT MR.KomTol <=====[ ]=====> 0814622392081 <=====[
*/
...
/******** OWNER NUMBER**********/
const ownerNumber = ["6285758779973@s.whatsapp.net","6285758779973@s.whatsapp.net"] 
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('🍁','white'),color('♥','red'),color('🍁','white'),color('JANGAN LUPA SUBS CHANEL','white'),color('YOU','red'),color('TUBE','white'),color('Mr.A43G','yellow'))
})
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Akhir Nya Beban Keluarga Berkurang Semoga Di Temani Kakek Sugiono :V @${num.split('@')[0]} `
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	// ANTI LINK GRUP
		        ...
		        setTimeout( () => {
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*NICO HARUS JADI ADMIN❗*`)})
		        }, 3000)
		        setTimeout( () => {
			        client.updatePresence(from, Presence.composing)
			        reply("Hedsot....")
			...
			case 'runtime':
				...
				reply(`Bot Nico Telah Aktif Selama\n*${kyun(uptime)}*`)
				break
				case 'totaluser':
					client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(ind.groupo())
					teks = `╭────「 *TOTAL USER NICO* 」\n`
					no = 0
...
85. Oʀᴇ ɴᴏ Iᴍᴏᴜᴛᴏ ɢᴀ Kᴏɴɴᴀɴɪ Kᴀᴡᴀɪɪ Wᴀᴋᴇ ɢᴀ Nᴀɪ`, text, tescuk, cr)
		...
		if (isBanned) return reply(ind.baned())
                ...
                anug = getRegisteredRandomId(_registered).replace('@s.whatsapp.net','')
                await reply('Mencari Parter Nico')
                await reply(`wa.me/${anug}`)
                await reply( `Partner Ketemu😉: 🗿\n*${prefix}next* — New partner Nico`)
            break
		case 'report':
                    ...
                    reply('*🍁DEAR USER🍁*\n*━━━━━━━━━━━━━━━━━━━━*\nMasalah telah di laporkan ke owner BOT NICO, laporan palsu/main² tidak akan ditanggapi.\n*━━━━━━━━━━━━━━━━━━━━*')
                    break
            case 'next':
		if (isBanned) return reply(ind.baned())
                if (!isRegistered) return reply( ind.noregis())
			...
			break
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    try {
						...
						buffer = await getBuffer(res.image)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '*HUSBU FROM NICO BOT*'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('*ERROR* ❌')
					}
...
break
				...
				case 'bcgc':
					...
					pok = await getBuffer(nimek)
					client.sendMessage(from, pok, image, { quoted: mek, caption: `*PINTEREST NICO BOT*`})
					await limitAdd(sender)
					break 
					case 'resepmasakan':
					if (!isRegistered) return reply(ind.noregis())
   				...
   				if (!isRegistered) return reply(ind.noregis())
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list GROUP BOT NICO :\n\n\`\`\``
					no = 0
                ...
                if (umurUser > 20) return reply(`*UMUR KAMU TERLALU TUA UNTUK MENJADI TEMAN NICO*`)
                if (umurUser < 10) return reply(`*UMUR KAMU TERLALU MUDA UNTUK MRNJADI TEMAN NICO*`)
                veri = sender
                if (isGroup) {
		    ...
		    reply('Terima Kasih Sudah Daftar')
            	...
            	case 'mining':
                      ...
                      reply(`karena kamu owner bot nico ini ${one}Xp untuk kamu😉`)
                      }else{
          ...
          case 'ping':
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				if (!isGroupAdmins) return reply(ind.admin())
					...
					anu = await fetchJson(`https://ferdiz-api.herokuapp.com/api/ytv?url=`, {method: 'get'})
					if (anu.error) return reply(anu.error)
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ *BROADCAST NICO* ❯\n\n${body.slice(4)}`})
						}
